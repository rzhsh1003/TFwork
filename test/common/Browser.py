...
FIREFOXRIVER_PATH = DRIVER_PATH + '\geckodriver.exe'
...
TYPES = {'firefox': webdriver.Firefox, 'chrome': webdriver.Chrome,'chrome-headless': webdriver.Chrome, 'ie': webdriver.Ie, 'phantomjs': webdriver.PhantomJS}
EXECUTABLE_PATH = {'firefox': FIREFOXRIVER_PATH, 'chrome': CHROMEDRIVER_PATH, 'chrome-headless': CHROMEDRIVER_PATH,'ie': IEDRIVER_PATH, 'phantomjs': PHANTOMJSDRIVER_PATH}
...
class Browser(object):
    ...
    def get(self, url, maximize_window=False, implicitly_wait=30):
        ...
        if self._type == 'chrome-headless':
            #chrome的headless模式
            chrome_options = Options()
            chrome_options.add_argument('--headless')
            self.driver = self.browser(executable_path=EXECUTABLE_PATH[self._type],chrome_options=chrome_options)
        else:
...
if __name__ == '__main__':
    b = Browser(browser_type='chrome-headless').get('http://www.baidu.com')
