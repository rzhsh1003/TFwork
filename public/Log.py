...
class Logger(object):
    def __init__(self, logger_name='TFwork'):
        self.logger = logging.getLogger(logger_name)
        logging.root.setLevel(logging.NOTSET)
        c = Config().get('log')
        self.log_file_name = c.get('file_name') if c and c.get('file_name') else 'logs.log'  # 日志文件
        ...
        # 日志输出格式
        pattern = c.get('pattern') if c and c.get('pattern') else '[%(levelname)s - %(asctime)s - %(name)s] : %(message)s'
